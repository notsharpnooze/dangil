...
def create_quote():
    ...
    print(f"{'Producto':<20} {'Cantidad':<10} {'Precio':<10} {'Subtotal':<12}")
    print("-" * 60)
    total = 0.0
    for item in quote_items:
        ...
        cantidad = int(item[1])
        try:
            precio_unit = float(str(item[2]).replace(',', '.'))
        except ValueError:
            precio_unit = 0.0
        subtotal = cantidad * precio_unit
        total += subtotal
        print(f"{nombre:<20} {cantidad:<10} {precio_unit:<10.2f} {subtotal:<12.2f}")
    print("-" * 60)
    print(f"{'TOTAL':<20} {'':<10} {'':<10} {total:<12.2f}")
    print()
    input("Presiona Enter para continuar...")
    ...
    if save == "s":
        ...
        # Agrega timestamp para evitar sobrescribir
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        filename = f"database/quotes_saved/{safe_desc}_{timestamp}.csv"
...
def view_quotes():
    ...
    while True:
        ...
        if len(rows) > 1:
            ...
            for item in data:
                ...
                try:
                    ...
                    precio = float(str(item[2]).replace(',', '.'))
                    subtotal = cantidad * precio
                    total += subtotal
                except (ValueError, IndexError):
                    continue
            print("-" * 45)
            print(f"{'TOTAL':<20} {'':<10} {total:<10.2f}")  # <-- Muestra el total
            print()
        else:
        ...
        if action == "1":
            ...
            # Extrae solo los productos (asumiendo encabezado ["Producto", "Cantidad", "Precio"])
            products = [row for row in quote_rows[1:] if len(row) == 3]

            # Llama a la función unificada
            save_order(order_id, client, date, description, products)

            # Guarda el archivo de descripción en ind_desc
            desc_dir = "database/orders/ind_desc"
            os.makedirs(desc_dir, exist_ok=True)
            desc_path = os.path.join(desc_dir, f"{order_id}_order.txt")
            with open(desc_path, "w", encoding="utf-8") as f:
                f.write(f"ID: {order_id}\nCliente: {client}\nFecha: {date}\nDescripción: {description}\n")

            print(f"Orden creada exitosamente con ID '{order_id}'.")
            input("Presiona Enter para continuar...") 
